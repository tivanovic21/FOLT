...
import edu.unizg.foi.nwtis.tivanovic21.vjezba_07_dz_2.dao.ObracunDAO;
import edu.unizg.foi.nwtis.tivanovic21.vjezba_07_dz_2.dao.PartnerDAO;
...
public class TvrtkaResource {
	...
	public Response getJelovnici() {
	    ...
	    try (var vezaBP = restConfiguration.dajVezu()) {
	        ...
	        try {
	            ...
	            List<Partner> filtriraniPartneri = partneriDB.stream()
	                .filter(p -> serverIds.contains(p.id()))
	                .collect(Collectors.toList());
	                ...
	                if (response != null && !response.startsWith("ERROR")) {
	                    try {
	                        ...
	                        Logger.getLogger(getClass().getName())
	                            .log(Level.WARNING, "Nepravilan JSON " + p.id(), ex);
	                    }
	                } else {
	                    Logger.getLogger(getClass().getName())
	                        .log(Level.WARNING, "Error getting jelovnik for partner " + p.id() + ": " + response);
	                }
	            }
	            ...
	            Logger.getLogger(getClass().getName())
	                .log(Level.SEVERE, "Error parsing server response", parseException);
	            return Response.status(Response.Status.INTERNAL_SERVER_ERROR).build();
	        }
	    ...
	    try (var vezaBP = this.restConfiguration.dajVezu()) {
	        ...
	        if (response != null && !response.startsWith("ERROR")) {
	            try {
	                ...
	                Logger.getLogger(getClass().getName())
	                    .log(Level.WARNING, "Nepravilan JSON za partnera " + partner.id(), ex);
	                return Response.status(Response.Status.INTERNAL_SERVER_ERROR).build();
	            }
	        } else {
	            Logger.getLogger(getClass().getName())
	                .log(Level.WARNING, "Greška prilikom dohvaćanja jelovnika za partnera " + partner.id() + ": " + response);
	            return Response.status(Response.Status.INTERNAL_SERVER_ERROR).build();
	        }
	...
	public Response getKartaPica() {
	    ...
	    try (var vezaBP = restConfiguration.dajVezu()) {
	        ...
	        PartnerPopis[] partneriArray = gson.fromJson(jsonResponse, PartnerPopis[].class);
	        Set<Integer> serverIds = Arrays.stream(partneriArray)
	                                       .map(PartnerPopis::id)
	                                       .collect(Collectors.toSet());

	        List<Partner> filtrirani = partneriDB.stream()
	            .filter(p -> serverIds.contains(p.id()))
	            .collect(Collectors.toList());
	        ...
	        for (Partner p : filtrirani) {
	            ...
	            if (response != null && !response.startsWith("ERROR")) {
	                try {
	                    ...
	                    Logger.getLogger(getClass().getName())
	                          .log(Level.WARNING, "Nepravilan JSON za partnera " + p.id(), ex);
	                }
	            } else {
	                Logger.getLogger(getClass().getName())
	                      .log(Level.WARNING, "Greška prilikom dohvaćanja karte pića za partnera " + p.id()
	                         + ": " + response);
	            }
	        }
