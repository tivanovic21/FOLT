...
/**
 * Klasa za konfiguraciju s tekstualnim zapisom podataka (Properties) u
 * datoteku..
 */
public non-sealed class KonfiguracijaTxt extends KonfiguracijaApstraktna {
	...
	public void spremiKonfiguraciju(String datotekaNaziv) throws NeispravnaKonfiguracija {
		...
		if (tip == null || tip.compareTo(KonfiguracijaTxt.TIP) != 0) {
			throw new NeispravnaKonfiguracija("Datoteka: " + datotekaNaziv + " nema tip: " + KonfiguracijaTxt.TIP);
		} else if (Files.exists(datoteka) && (!Files.isRegularFile(datoteka) || !Files.isWritable(datoteka))) {
			throw new NeispravnaKonfiguracija("Datoteka: " + datotekaNaziv + " nije datoteka/ne može se u nju pisati");
		}
		try (var writer = new OutputStreamWriter(Files.newOutputStream(datoteka), "utf8")) {
	...
	public void ucitajKonfiguraciju() throws NeispravnaKonfiguracija {
		...
		if (tip == null || tip.compareTo(KonfiguracijaTxt.TIP) != 0) {
			throw new NeispravnaKonfiguracija("Datoteka: " + this.nazivDatoteke + " nema tip: " + KonfiguracijaTxt.TIP);
		} else if (!Files.exists(datoteka) || !Files.isRegularFile(datoteka) || !Files.isReadable(datoteka)) {
			throw new NeispravnaKonfiguracija(
					"Datoteka: " + this.nazivDatoteke + " nije ispravnog tipa/ne postoji/ne može se učitati");
		}
		try (var reader = new InputStreamReader(Files.newInputStream(datoteka), "utf8")) {
			this.postavke.load(reader);
		} catch (IOException ex) {
			throw new NeispravnaKonfiguracija("Problem kod učitavanja datoteke: '" + this.nazivDatoteke + "'.");
		}
	}
